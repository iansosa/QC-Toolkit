...
class Handler():
    ...
    def SaveGeometry(self,decour="geom",path=None,instruct=False,charges=None,decour_charges="charges",dir='',para=0): #saves the geometry to a gen file in angstroms
        print("Saving geometry..")
        angstrom = 0.529177249
        folder = dir + "DFTB+"+('_para%d' % para)*(para!=0)
        if instruct == True:
        ...
        if charges != None:
            ...
            with open("DFTB+"+('_para%d' % para)*(para!=0)+"/charges.dat", 'w') as f:
                f.write("           6\n")
    ...
    def LoadGeometry(self,path="geom.out.gen",dir=''): #loads the geometry from a gen, xyz or sdf file in angstroms and converts it into Bohr
        ...
        if extension == "sdf":
            ...
            self.Nat, geometry = filetypes.LoadGeometrysdf("SavedStructures/"+path,angstrom)
    ...
    def RunStatic(self,vdw=None,read_charges=False,packing=[5,5,5],mbdpacking=[2,2,2],dir='',para=0):
        path_to_DFTB = dir + 'DFTB+' + ('_para%d' % para)*(para!=0)
        if self.periodic == False:
            if vdw == None:
                shutil.copyfile('in_files/static_calc.hsd', path_to_DFTB+'/dftb_in.hsd')
            elif vdw == "MBD":
                shutil.copyfile('in_files/static_calc_mbd.hsd', path_to_DFTB+'/dftb_in.hsd')
            elif vdw == "PW":
                shutil.copyfile('in_files/static_calc_pw.hsd', path_to_DFTB+'/dftb_in.hsd')
            elif vdw == "TS":
                shutil.copyfile('in_files/static_calc_ts.hsd', path_to_DFTB+'/dftb_in.hsd')
            else:
                print ("Dispersion type not recognized")
                sys.exit()
        else:
            if vdw == None:
                shutil.copyfile('in_files/static_calc-periodic.hsd', path_to_DFTB+'/dftb_in.hsd')
            elif vdw == "MBD":
                shutil.copyfile('in_files/static_calc-periodic_mbd.hsd', path_to_DFTB+'/dftb_in.hsd')
            elif vdw == "PW":
                shutil.copyfile('in_files/static_calc-periodic_pw.hsd', path_to_DFTB+'/dftb_in.hsd')
            elif vdw == "TS":
                shutil.copyfile('in_files/static_calc-periodic_ts.hsd', path_to_DFTB+'/dftb_in.hsd')
            else:
                print ("Dispersion type not recognized")
                sys.exit()

        try:
            file = open(path_to_DFTB+"/dftb_in.hsd", "r+")
        except OSError:
            print ("Could not open dftb_in.hsd file")
            sys.exit()
        lines = file.readlines()
        ...
        with open(path_to_DFTB+'/dftb_in.hsd', 'w') as f:
            for i in range(len(lines)):
                f.write(lines[i])
                
        subprocess.run("./" + path_to_DFTB+"/dftbOpt.sh", shell=True)
    ...
    def GetForces(self,dir='',para=0): #load all of the total forces from DFTB
        try:
            file = open(dir + "DFTB+" + ('_para%d' % para)*(para!=0) + "/detailed.out", "r+")
        except OSError:
