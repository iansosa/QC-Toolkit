...
class MDHandler():
    ...
    def RunMD(self,steps, dir='',temp=300,vdw=None,keepstationary=False,static=None,save_steps=1, loadV=False,
              read_charges=False,seed=0,para=0):
        path_to_DFTB = dir + 'DFTB+' + ('_para%d' % para) * (para != 0)
        if vdw == None:
            shutil.copyfile('in_files/md.hsd', path_to_DFTB+'/dftb_in.hsd')
        elif vdw == "MBD":
            shutil.copyfile('in_files/md_mbd.hsd',path_to_DFTB+'/dftb_in.hsd')
        elif vdw == "PW":
            shutil.copyfile('in_files/md_pw.hsd', path_to_DFTB+'/dftb_in.hsd')
        elif vdw == "TS":
            shutil.copyfile('in_files/md_ts.hsd', path_to_DFTB+'/dftb_in.hsd')
        else:
        ...
        if seed > 0:
            idx = -1
            for i in range(len(lines)):
                if lines[i].find("RandomSeed") != -1:
                    idx = i
            targetline = "  RandomSeed = %d\n" % seed
            lines[idx] = targetline

        if loadV:
            idx = -1
            _,_,v_initial = filetypes.Loadxyz_raw(dir+'DFTB+/geo_end.xyz',save_iter=[-1,])
            v_initial = v_initial[0]
            targetline = ' Velocities [A/ps] = {'
            for i in range(len(lines)):
                if lines[i].find("Velocities") != -1:
                    idx = i
            for j in range(len(v_initial)):
                targetline = targetline + '\n' + \
                             ' ' + str(v_initial[j][0]) + \
                             ' ' + str(v_initial[j][1]) + \
                             ' ' + str(v_initial[j][2])
    ...
    def RunStaticOnFrame(self,idx,vdw=None):
    ...
    def LoadEvolution(self,path=None):
        ...
        if path == None:
            ...
            Nat, Niter, self.evolution = filetypes.Loadxyz(path,angstrom)
        self.acelerations = []
